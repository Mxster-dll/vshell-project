...
class WebviewWindowsPlugin : public flutter::Plugin {
 ...
 private:
  ...
  // Find the Flutter main window of this process (class
  // FLUTTER_RUNNER_WIN32_WINDOW) to host WebView2 composition - fixes IME
  // candidate position (see CreateWebviewInstance)
...
// static
HWND WebviewWindowsPlugin::FindFlutterMainWindow() {
  struct FindState {
    HWND result;
    DWORD pid;
  };
  FindState state{nullptr, GetCurrentProcessId()};
  EnumWindows(
      [](HWND h, LPARAM l) -> BOOL {
        auto* st = reinterpret_cast<FindState*>(l);
        DWORD wpid = 0;
        GetWindowThreadProcessId(h, &wpid);
        if (wpid != st->pid) return TRUE;
        wchar_t cls[128];
        if (GetClassNameW(h, cls, 128) > 0 &&
            wcscmp(cls, L"FLUTTER_RUNNER_WIN32_WINDOW") == 0) {
          st->result = h;
          return FALSE;
        }
        return TRUE;
...
void WebviewWindowsPlugin::CreateWebviewInstance(
  ...
  // Host window: prefer the Flutter main window (class FLUTTER_RUNNER_WIN32_WINDOW)
  // to fix IME candidate position. The original message-only host window
  // (FlutterWebviewMessage) got foreground via Chromium SetForegroundWindow,
  // deactivating the main window -> TSF candidate fallback to screen bottom
  // (WebView2Feedback #2241 family). With the main window as host, clicking the
  // client area activates the main window itself. Fall back to a message-only
  // window (owns_host_window=true, destroyed by plugin) if not found.
  HWND host = FindFlutterMainWindow();
  ...
  if (host) {
    ...
    owns_host_window = true;
  }

  std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>>
      shared_result = std::move(result);
  webview_host_->CreateWebview(
      hwnd, owns_host_window, true,
