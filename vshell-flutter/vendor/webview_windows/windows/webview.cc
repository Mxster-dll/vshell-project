...
void Webview::SendScroll(double delta, bool horizontal) {
  // FIX(vshell): Flutter reports scaled deltas (e.g. 66.67 per notch at
  // DPI 1.5). Accumulate fractional WHEEL_DELTA steps so that both notch
  // mice (66.67 -> ~3.33 steps/event) and smooth-scroll mice (small
  // high-frequency deltas) scroll reliably; emit only whole 120s.
  constexpr double kStepsPerDelta = 6.0 / 120.0;
  double& acc = horizontal ? h_scroll_acc_ : v_scroll_acc_;
  acc += delta * kStepsPerDelta;
  auto steps = static_cast<int>(std::floor(acc + 0.5));
  if (steps == 0) {
    return;
  }
  if (steps > 20) steps = 20;   // clamp: extreme fast-wheel bursts
  if (steps < -20) steps = -20;
  acc -= steps;
  auto offset = static_cast<short>(steps * 120);

  // TEMP-DIAG
  {
    std::ofstream f("D:/vshell_scroll_native.log", std::ios::app);
    if (f) {
      f << "d=" << delta << " h=" << (horizontal ? 1 : 0) << " steps=" << steps
        << " off=" << (int)offset << " pt=" << last_cursor_pos_.x << ","
        << last_cursor_pos_.y << "\n";
    }
  }

  // FIX(vshell): original code used point={0,0}, which sent the wheel
  // event to the top-left corner of the page -- no scrollable container
  // under the cursor -> the whole page did not respond to mouse wheel.
  // Use last_cursor_pos_ instead (same client-coord system as click/move).
  POINT point;
  point.x = last_cursor_pos_.x;
  point.y = last_cursor_pos_.y;
