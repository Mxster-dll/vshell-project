...
class Webview {
 ...
 private:
  ...
  // FIX(vshell): fractional wheel accumulators -- smooth-scroll mice emit
  // high-frequency small deltas that would round to zero steps per event;
  // remainder carries across events so any amount of wheel movement scrolls.
  double v_scroll_acc_ = 0.0;
  double h_scroll_acc_ = 0.0;
